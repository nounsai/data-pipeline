...
for row in cur.fetchall():
    if is_question(row['content']):
       ...
       SELECT %s WHERE NOT EXISTS (SELECT 1 FROM qa2_staging WHERE message_id = %s);
       """

    #    print(f"Inserting question:  {row['content']}")
        cur.execute(insert_query, (row['id'], row['id']))
        inserted = cur.rowcount > 0
        print(
            f"{'Inserted' if inserted else 'Already exists'}: {row['content']}")
